...
function parameter_selection()
        ...
        beta_Bare = rand(0.1: 0.0001: 2.0)
        beta_Forest = rand(0.1: 0.0001: 2.0)
        beta_Grass = rand(0.1: 0.0001: 2.0)
        beta_Rip = rand(0.1: 0.0001: 2.0)
        Ce = rand(0.4:0.0001:0.8)
        Drainagecapacity = 0.0
        Interceptioncapacity_Bare = 0.0
        Interceptioncapacity_Forest = rand(1.0:0.00001:3.0)
        ...
        if Interceptioncapacity_Forest < max_Interceptioncapacity_Grass
                Interceptioncapacity_Grass = rand(1.0:0.00001:Interceptioncapacity_Forest)
        else
                Interceptioncapacity_Grass = rand(1.0:0.00001: max_Interceptioncapacity_Grass)
        end

        if Interceptioncapacity_Forest < max_Interceptioncapacity_Rip
                Interceptioncapacity_Rip = rand(1.0:0.00001:Interceptioncapacity_Forest)
        else
                Interceptioncapacity_Rip = rand(1.0:0.00001: max_Interceptioncapacity_Rip)
        end
        # parameter constraints on fast reservoir coefficients
        Kf_Rip = rand(0.5:0.00001:3.0)
        if Kf_Rip < max_Kf
                Kf = rand(0.1:0.00001:Kf_Rip)
        else
                Kf = rand(0.1:0.00001: max_Kf)
        end

        if Kf < max_Ks
                Ks = rand(0.001:0.000001: Kf)
        else
                Ks = rand(0.001:0.000001: max_Ks)
        end
        Meltfactor = rand(1.75:0.001:6.0)
        Mm = rand(0.001:0.001:1.0)
        Precipitation_Gradient = 0.0
        #Precipitation_Gradient = round(random_parameter(0, 0.0045), digits= 5)
        Ratio_Pref = rand(0.0:0.0001:1.0)
        # Parameter Constrain SOilstoragecapacity Forest >= Grass >= Rip/Bare
        Soilstoaragecapacity_Forest = rand(100.0:0.01:500.0)
        if Soilstoaragecapacity_Forest < max_Soilstoaragecapacity_Grass
                Soilstoaragecapacity_Grass = rand(50.0:0.01:Soilstoaragecapacity_Forest)
        else
                Soilstoaragecapacity_Grass = rand(50.0:0.01: max_Soilstoaragecapacity_Grass)
        end

        if Soilstoaragecapacity_Grass < max_Soilstoaragecapacity_Rip
                Soilstoaragecapacity_Rip = rand(50.0:0.01:Soilstoaragecapacity_Grass)
        else
                Soilstoaragecapacity_Rip = rand(50.0:0.01: max_Soilstoaragecapacity_Rip)
        end

        if Soilstoaragecapacity_Grass < max_Soilstoaragecapacity_Bare
                Soilstoaragecapacity_Bare = rand(5.0:0.01:Soilstoaragecapacity_Grass)
        else
                Soilstoaragecapacity_Bare = rand(50.0:0.01: max_Soilstoaragecapacity_Bare)
        end

        Temp_Thresh = rand(-2.0:0.001:2.0)
        Ratio_Riparian = rand(0.05:0.001:0.5)
