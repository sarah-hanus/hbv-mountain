#function run_Projections(ID, nmax)
using CSV

local_path = "/home/sarah/"
# ------------ CATCHMENT SPECIFIC INPUTS----------------
Areas = readdlm(local_path*"HBVModel/Gailtal/Projections/areas.csv", skipstart=1, ',')
ID_Prec_Zones = (Areas[:,1])
...
#Skipto = [24, 22, 22, 22]
# get the area of all elevation zones in the HRUs in the precipitation zones
# first column gives all elevations next columns give bare, forest, grass, rip
Areas_HRUs =  readdlm(local_path*"HBVModel/Gailtal/Projections/HRU_Elevations.csv", skipstart=2, ',')
Area_HRU_Each_Prec_Zone = transpose(sum(Areas_HRUs[:,2:end], dims=1))
Sum_Area_Prec_Zone = Float64[]
for i in 1:length(ID_Prec_Zones)
        Current_Perc_HRU = sum(Area_HRU_Each_Prec_Zone[1+4*(i-1):4*(i)])
        append!(Sum_Area_Prec_Zone, Current_Perc_HRU)
        append!(Sum_Area_Prec_Zone, Current_Perc_HRU)
        append!(Sum_Area_Prec_Zone, Current_Perc_HRU)
        append!(Sum_Area_Prec_Zone, Current_Perc_HRU)
end
# get the percentage of each HRU of the precipitation zone
Percentage_HRU = Area_HRU_Each_Prec_Zone ./ Sum_Area_Prec_Zone

#Percentage_HRU = CSV.read(local_path*"HBVModel/Gailtal/HRUPercentage.csv", header=[1], decimal=',', delim = ';')
Elevation_Catchment = convert(Vector, Areas_HRUs[:,1])
startyear = 1983
endyear = 2005
...
Total_Discharge = zeros(length(Timeseries))
...
index_Coordinates_Prec = readdlm("Gailtal/Projections/index_coordinates.csv", ',', skipblanks=true)
length_Coordinates = [7,11,11,11,11,10]
...
for i in 1: length(ID_Prec_Zones)
        ...
        #function for getting prec data
        function getPrec(path_to_prec, ID_Prec_Zone, index_Coordinates_Prec_Zones, length_Coordinates, startindex, endindex)
            Projections_Precipitation = readdlm(path_to_prec, ',')
            Precipitation_Zone = zeros(size(Projections_Precipitation)[1])
            for i in 1: length(index_Coordinates_Prec_Zones[1:length_Coordinates])
        ...
        Precipitation = getPrec(local_path*"HBVModel/Gailtal/Projections/pr_sim1.txt", Int(i), index_Coordinates_Prec[i,:], length_Coordinates[i], indexstart_Proj, indexend_Proj)
