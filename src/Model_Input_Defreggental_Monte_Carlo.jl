...
@everywhere function run_MC(ID, nmax)
        ...
        for n in 1 : nmax
                ...
                if Goodness_Fit != -9999
                        ...
                        if size(All_Goodness)[2]-1 == 100
                                ...
                                if count != 100
                                        open(local_path*"HBVModel/Defreggental_Parameterfit_"*string(ID)*"_"*string(number_Files)*".csv", "a") do io
        ...
        All_Goodness = transpose(All_Goodness[:, 2:end])
        open(local_path*"HBVModel/Defreggental_Parameterfit_"*string(ID)*".csv", "a") do io
                writedlm(io, All_Goodness,",")
        end
end
#
