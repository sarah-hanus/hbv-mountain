...
Timeseries_whole = collect(Date(startyear, 1, 1):Day(1):Date(endyear,12,31))
function convertDischarge(Discharge, Area)
        Discharge_mm = Discharge / Area * (24 * 3600 * 1000)
        return Discharge_mm
end
Temperature_Mean_Elevation = Temperature_Elevation_Catchment[:,findfirst(x-> x==2100, Elevation_Zone_Catchment)]
...
start = findfirst(x->x == Date(startyear, 1, 1), Timeseries_whole)
ending = findfirst(x->x == Date(endyear,12,31), Timeseries_whole)
...
daily_WB, WB, Total_WB, Annual_Prec, Annual_Epot, Annual_Discharge = checkwaterbalance(Total_Precipitation[start:ending], Observed_Discharge_diff, Potential_Evaporation[start:ending], Area_Catchment)
