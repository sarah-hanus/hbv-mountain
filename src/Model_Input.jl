...
Dates_Sample = Dates[in([1950:1980]).(Dates.Year), :]
Precipitation_Sample = Precipitation[:,25]
Temp_Sample = Temperature[:,25]/10 #makes a vector
Precipitation_Sample = Precipitation_Sample[1:3650]/10
Temp_Sample = Temp_Sample[1:3650]


Nr_Elevationbands, Prec_Elevation, Temp_Elevation = getelevationdata(250, 1900, 2238, 2400, 0.003, Temp_Sample, Precipitation_Sample)

Potential_Evaporation = rand(3650, Nr_Elevationbands) * 10
Potential_Evaporation_Mean = Potential_Evaporation[:,1]

Areas = [0.51, 0.22, 0.26, 0.01]

bare_input = HRU_Input(ones(10)/10, Areas[1], 0.0, Nr_Elevationbands, [0], 0, [0], 0, [0], 0, 0)
forest_input = HRU_Input(ones(10)/10, Areas[2], 0, Nr_Elevationbands,[0], 0, [0], 0, [0],  0, 0)
grass_input = HRU_Input(ones(10)/10, Areas[3], 0, Nr_Elevationbands, [0], 0, [0], 0, [0],  0, 0)
rip_input = HRU_Input(ones(10)/10, Areas[4], 0, Nr_Elevationbands, [0], 0, [0], 0, [0],  0, 0)
