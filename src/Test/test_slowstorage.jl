...
@testset "slowstorage" begin
    for i in 1:10000
        ...
        @test round(Storage - Slowstorage, digits=prec) == round(Slow_Discharge, digits=prec)
        ...
        @test round((Storage + GWflow) * Ks, digits=prec)  == round(Slow_Discharge + Riparian_Discharge, digits = prec)
        ...
        @test round(GWflow * Ks, digits=prec) == round(Slow_Discharge + Riparian_Discharge, digits=prec)
        # if storage empty before, input = output+ new storage
        @test round(Slow_Discharge + Riparian_Discharge + Slowstorage, digits=prec) == round(GWflow, digits=prec)
    end
end
